...
{
	if (!(((m_Area.x + m_XDirection) < 0 || (m_Area.x + m_Area.w + m_XDirection) >= MAP_WIDTH) || Map.TouchWall(m_Area, m_XDirection, m_YDirection)))
	{
		m_Area.x += m_XDirection;
	}
	if (!((m_Area.y + m_YDirection) < 0 || ((m_Area.y + m_Area.h + m_YDirection) >= MAP_HEIGHT) || Map.TouchWall(m_Area, m_XDirection, m_YDirection)))
	{
		m_Area.y += m_YDirection;
	}
}
